         ...
         || $scope === PHS_Scope::SCOPE_REMOTE) {
            ...
            if ($scope === PHS_Scope::SCOPE_API ) {
                ...
                if (!empty($response_data['http_code_is_error']) ) {
                    PHS_Model_Api_monitor::api_request_error( $response_data['http_code'],
                        (!empty($response_params['error']['code'])?$response_params['error']['code'].': ':'').
                        ($response_params['error']['message'] ?? 'Unknown error.')
                    );
                } else {
                    PHS_Model_Api_monitor::api_request_success($response_data['http_code']);
                }
            }
