...
class PHS_Jwt extends PHS_Library
{
    ...
    public function decode( $jwt, $params = false )
    {
        ...
        if( empty( $params['allowed_algs'] ) or !is_array( $params['allowed_algs'] ) )
            $params['allowed_algs'] = array();
        ...
        if( !is_array( $params['audience'] ) )
            $params['audience'] = array( $params['audience'] );
        ...
        if( empty( $jwt )
         or !($segments_arr = @explode( '.', $jwt ))
         or !is_array( $segments_arr )
         or count( $segments_arr ) != 3 )
        {
        ...
        if( !($header_str = $this->_safe_base64_decode( $header_enc ))
         or !($header_arr = @json_decode( $header_str, true ))
         or !is_array( $header_arr ) )
        {
        ...
        if( !($payload_str = $this->_safe_base64_decode( $payload_enc ))
         or null === ($payload_arr = @json_decode( $payload_str, true )) )
        {
        ...
        if( empty( $header_arr['alg'] )
         or empty( self::$supported_algs[$header_arr['alg']] ) )
        {
        ...
        and !in_array( $header_arr['alg'], $params['allowed_algs'] ) )
        {
        ...
        if( isset( $payload_arr['nbf'] ) and $payload_arr['nbf'] > ($timestamp + $leeway) )
        {
        ...
        if( isset( $payload_arr['iat'] ) and $payload_arr['iat'] > ($timestamp + $leeway) )
        {
            $this->set_error( self::ERR_PARAMETERS, $this->_pt( 'JWT cannot be used now.' ) );
            return false;
        }
        ...
        if( isset( $payload_arr['exp'] ) and ($timestamp - $leeway) > $payload_arr['exp'] )
        {
        ...
        if( (empty( $payload_arr['aud'] ) and !empty( $params['audience'] ))
         or (!empty( $payload_arr['aud'] )
                and (empty( $params['audience'] ) or !in_array( $payload_arr['aud'], $params['audience'], true ))
            ) )
        {
        ...
        if( !empty( $params['issuer'] )
        and (empty( $payload_arr['iss'] ) or $payload_arr['iss'] !== $params['issuer']) )
        {
    ...
    public function verify_payload( $msg, $signature, $key, $alg )
    {
        ...
        $alg = strtoupper( $alg );
        if( empty( $alg )
         or empty( self::$supported_algs[$alg] ) )
        {
            $this->set_error( self::ERR_PARAMETERS, $this->_pt( 'JWT algorithm not supported.' ) );
            return false;
        }
    ...
    public function encode( $payload, $key, $params = false )
    {
        ...
        if( empty( $params['alg'] )
         or empty( self::$supported_algs[$params['alg']] ) )
        {
            $this->set_error( self::ERR_PARAMETERS, $this->_pt( 'JWT algorithm not supported.' ) );
            return false;
        }
