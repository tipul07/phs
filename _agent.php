<?php
    ...
    {
        PHS_Logger::logf( '[ERROR] Error instantiating PHS_Agent class!', PHS_Logger::TYPE_AGENT );
        exit;
    }

    PHS_Logger::logf( "\n".' --- Started agent...', PHS_Logger::TYPE_AGENT );

    PHS_Logger::logf( ' - Checking stalling jobs...', PHS_Logger::TYPE_AGENT );

    if( !($run_result = $agent_obj->check_stalling_agent_jobs()) )
    {
        PHS_Logger::logf( 'Error checking stalling agent jobs!', PHS_Logger::TYPE_AGENT );
    ...
    {
        PHS_Logger::logf( 'Stalling agent jobs checked with success: '.$run_result['jobs_running'].' jobs running, '.
    ...
    PHS_Logger::logf( ' - Launching agent jobs...', PHS_Logger::TYPE_AGENT );
    ...
    {
        PHS_Logger::logf( 'DEBUG data: '.$debug_data['db_queries_count'].' queries,'.
