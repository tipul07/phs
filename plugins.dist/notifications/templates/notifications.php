...
if (!($ajax_placeholders_prefix = $this->view_var('ajax_placeholders_prefix'))
 || !is_string($ajax_placeholders_prefix)) {
    $ajax_placeholders_prefix = 'phs_ajax';
}

if (!empty($display_channels) && is_array($display_channels)) {
    $channel_to_class = ['success' => 'success', 'errors' => 'error', 'warnings' => 'warning'];
    foreach ($display_channels as $channel) {
        if (empty($notifications_arr[$channel])
         || !is_array($notifications_arr[$channel])) {
            continue;
        }
