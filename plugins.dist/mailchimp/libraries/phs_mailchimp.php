...
class Mailchimp extends PHS_Library
{
    ...
    public function create_list($list_arr)
    {
        ...
        if (empty($list_arr['contact']) || !is_array($list_arr['contact'])
         || empty($list_arr['contact']['company'])
         || empty($list_arr['contact']['address1'])
         || empty($list_arr['contact']['city'])
         || empty($list_arr['contact']['state'])
         || empty($list_arr['contact']['zip'])
         // country ISO 2 chars...
         || empty($list_arr['contact']['country'])
        ) {
        ...
        if (empty($list_arr['campaign_defaults']) || !is_array($list_arr['campaign_defaults'])) {
            $list_arr['campaign_defaults'] = [];
        }
    ...
    public function add_member_to_list($list_id, $member_arr)
    {
        ...
        if (empty($member_arr) || !is_array($member_arr)) {
    ...
    private function _do_call($payload = false, $params = false)
    {
        ...
        $api_params['userpass'] = ['user' => 'something', 'pass' => $this->_api_settings['api_key']];
