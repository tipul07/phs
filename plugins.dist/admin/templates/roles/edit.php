...
?>
<form id="edit_role_form" name="edit_role_form" method="post"
      action="<?php echo PHS::url( [ 'p' => 'admin', 'a' => 'role_edit' ], [ 'rid' => $this->view_var( 'rid' ) ] )?>">
    ...
    <div class="form_container">
        ...
        <fieldset class="form-group">
            ...
            <input type="text" id="name" name="name" class="form-control" required="required"
                   value="<?php echo form_str( $this->view_var( 'name' ) )?>" autocomplete="off" />
            </div>
        </fieldset>
        ...
        <fieldset class="form-group">
            ...
            <input type="text" id="description" name="description" class="form-control" required="required"
                   value="<?php echo form_str( $this->view_var( 'description' ) )?>" autocomplete="off" />
            </div>
        </fieldset>
            ...
            foreach( $role_units_by_slug as $unit_slug => $unit_arr )
            {
                ...
                <div class="clearfix">
                    <div style="float:left;">
                        <input type="checkbox" id="ru_slug_<?php echo $unit_slug ?>" name="ru_slugs[]"
                               value="<?php echo form_str( $unit_slug )?>" rel="skin_checkbox"
                            <?php echo (in_array( $unit_slug, $ru_slugs, true ) ? 'checked="checked"' : '')?> /></div>
                    <label style="margin-left:5px;width: auto !important;max-width: none !important;float:left;"
                           for="ru_slug_<?php echo $unit_slug ?>">
