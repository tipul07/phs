...
function phs_messages_summary_delete_message( id )
{
    ...
    var ajax_obj = PHS_JSEN.do_ajax( "<?php echo PHS_Ajax::url(['p' => 'messages', 'a' => 'inbox']); ?>", ajax_params );
}
</script><div id="messages-summary-popup">
<div id="messages-summary-popup-title">
    <a href="<?php echo PHS::url(['p' => 'messages', 'a' => 'inbox']); ?>"><i class="fa fa-envelope"></i><?php echo $this->_pt('Inbox'); ?></a> <span class="messages_counter">(<?php echo $messages_new; ?>/<?php echo $messages_count; ?>)</span>
    <?php
...
if (!empty($messages_list) && is_array($messages_list)) {
    foreach ($messages_list as $message_id => $full_message_arr) {
        $message_link = PHS::url(['p' => 'messages', 'a' => 'view_message'], ['muid' => $full_message_arr['message_user']['id']]);
