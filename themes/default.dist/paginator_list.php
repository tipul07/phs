...
&& !empty($columns_count)) {
    ...
    foreach ($columns_arr as $column_arr) {
        ...
        ?><th class="<?php echo (!empty($column_arr['sortable']) ? 'sortable' : '').' '.$column_arr['extra_classes']; ?>"
                  ...
                  <?php if (!empty($column_arr['column_colspan']) && $column_arr['column_colspan'] > 1) {
                      echo 'colspan="'.$column_arr['column_colspan'].'"';
                  } ?>
                  <?php if (!empty($column_arr['raw_attrs'])) {
                      echo $column_arr['raw_attrs'];
                  } ?>
            ><?php
...
&& $pagination_arr['max_pages'] > 1) {
    ...
    if ($pagination_arr['page'] > 0) {
        ?><a href="<?php echo add_url_params($url_without_page, [$page_var_name => ($pagination_arr['page'] - 1)]); ?>"><?php echo $this::_t('Previous page'); ?></a> | <?php
    }

    if ($left_start > 0) {
        ?><a href="<?php echo add_url_params($url_without_page, [$page_var_name => 0]); ?>">1</a> ... <?php
    }
    ...
    if ($right_end < $pagination_arr['max_pages']) {
        ?> ... <a href="<?php echo add_url_params($url_without_page, [$page_var_name => ($pagination_arr['max_pages'] - 1)]); ?>"><?php echo $pagination_arr['max_pages']; ?></a><?php
    }

    if ($pagination_arr['page'] + 1 < $pagination_arr['max_pages']) {
        ?> | <a href="<?php echo add_url_params($url_without_page, [$page_var_name => ($pagination_arr['page'] + 1)]); ?>"><?php echo $this::_t('Next page'); ?></a><?php
    }
