...
<li>
    <a href="javascript:void(0);" id="lang_chooser" title="<?php echo $this::_t('Select language'); ?>">
        <div class="arrow"></div>
        <?php
            if (!empty($defined_languages[$current_language]['flag_file']) && !empty($defined_languages[$current_language]['dir']) && !empty($defined_languages[$current_language]['www'])
            && @file_exists($defined_languages[$current_language]['dir'].$defined_languages[$current_language]['flag_file'])) {
...
foreach ($defined_languages as $lang => $lang_details) {
    $language_flag = '';
    if (!empty($lang_details['flag_file']) && !empty($lang_details['dir']) && !empty($lang_details['www'])
    && @file_exists($lang_details['dir'].$lang_details['flag_file'])) {
