...
var PHS_RActive_notifications = PHS_RActive_notifications || PHS_RActive.extend({
    ...
    phs_add_message_in_queue: function( msg, type, timeout = 5 ) {
        ...
        if( timeout > 0 ) {
            setTimeout(function () {
                inner_this.phs_remove_message_from_queue(msg_id, type)
            }, timeout * 1000);
        }
    },
    ...
    phs_remove_message_from_queue: function( id, type ) {
        ...
        if( !PHS_JSEN.object_has_keys( messages[type] ) ) {
            delete messages[type];
        }
        if( !PHS_JSEN.object_has_keys( messages ) ) {
            messages = {};
        }
